...
The Pages build script reuses `godot4` when the build environment already provides it, then installs the committed custom reduced-size Godot 4.5 Web templates from `tools/godot-export-templates/4.5.stable/`. It then runs the repo's headless test suite before exporting the Web build.
...
To regenerate the committed custom templates after changing the size profile:
